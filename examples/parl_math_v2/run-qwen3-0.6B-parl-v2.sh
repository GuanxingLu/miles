...
# PARL v2 debug run on Qwen3-0.6B (4x5880).
# True agent-spawn RL: orchestrator autonomously calls `consult_solvers`
# tool via miles' multi_turn.generate. Solvers run on the same SGLang engine.
...
# Pin the SGLang router address so the consult_solvers tool (running inside
# the rollout process) can reach it via env vars without plumbing args.
SGLANG_ROUTER_IP=${SGLANG_ROUTER_IP:-127.0.0.1}
SGLANG_ROUTER_PORT=${SGLANG_ROUTER_PORT:-18765}
export MILES_SGLANG_ROUTER_IP=${SGLANG_ROUTER_IP}
export MILES_SGLANG_ROUTER_PORT=${SGLANG_ROUTER_PORT}

source "${REPO_DIR}/scripts/models/qwen3-0.6B.sh"

CKPT_ARGS=(
   --hf-checkpoint ${MODEL_ROOT}/Qwen3-0.6B
   --ref-load ${MODEL_ROOT}/Qwen3-0.6B_torch_dist
   --load ${REPO_DIR}/saves/Qwen3-0.6B-parl-v2/${RUN_ID}
   --save ${REPO_DIR}/saves/Qwen3-0.6B-parl-v2/${RUN_ID}
   --save-interval 50
)
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-0.6B-parl-v2-math
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    \"PYTHONPATH\": \"${MEGATRON_PATH}:${REPO_DIR}\",
    \"CUDA_DEVICE_MAX_CONNECTIONS\": \"1\",
    \"NCCL_NVLS_ENABLE\": \"${HAS_NVLINK}\",
    \"MILES_SGLANG_ROUTER_IP\": \"${MILES_SGLANG_ROUTER_IP}\",
    \"MILES_SGLANG_ROUTER_PORT\": \"${MILES_SGLANG_ROUTER_PORT}\",
    \"http_proxy\": \"\",
    \"https_proxy\": \"\",
    \"all_proxy\": \"\",
    \"HTTP_PROXY\": \"\",
