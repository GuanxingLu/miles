...
This example demonstrates how to train a **Claude Code Agent** using Miles RL framework. The agent learns to perform software engineering tasks by interacting with a codebase using tools like `bash`, `read`, `write`, `edit`, `grep`, and `glob`.
...
├── README.md                          # This file
├── tool_executor.py                   # Claude Code tool execution (bash, read, write, etc.)
├── generate_with_claude_code.py       # Miles integration (generate & reward functions)
├── gym_server.py                      # Claude Code Gym environment server (TODO)
├── run-train.sh                       # Training launch script
├── sample_tasks.jsonl                 # Example task dataset
└── data/
    └── prepare_tasks.py               # Task dataset preparation utilities (TODO)
```
