...
# Qwen3-4B no-tool-call baseline for retool_v2.
# Runs standard single-turn RL (no tool specs, no multi-turn generation).
...
RUN_ARGS=(
   --mode "${MODE}"
   --run-id "${RUN_ID}"
   --save-path "${REPO_DIR}/saves/Qwen3-4B/${RUN_ID}"
)
...
python examples/retool_v2/run.py \
   ${MODEL_ARGS[@]} \
   ${RUN_ARGS[@]} \
   ${PARALLEL_ARGS[@]} \
   ${DATA_ARGS[@]}
