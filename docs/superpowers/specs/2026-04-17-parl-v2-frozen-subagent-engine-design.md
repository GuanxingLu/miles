...
`examples/parl_math_v2/sglang_config_4B.yaml`
```yaml
sglang:
  - name: actor
    update_weights: true
    server_groups:
      - worker_type: regular
        num_gpus: 6                   # 配合 --rollout-num-gpus 7
  - name: subagent
...
sglang:
  - name: actor
    ...
    server_groups:
      - worker_type: regular
        num_gpus: 3                   # 配合 --rollout-num-gpus 4
  - name: subagent
    update_weights: false
    server_groups:
      - worker_type: regular
...
具体 `num_gpus` 数字以两个 launch 脚本实际的 `--rollout-num-gpus` 为准（总和必须相等，否则 `_resolve_sglang_config:1103` assert 失败）。
